...
function makeGUI() {
  ...
  let usePSO = d3.select("#usePSO").on("change", function () {
    state.usePSO = this.checked;
    state.serialize();
    userHasInteracted();
    parametersChanged = true;
  });
  ...
  psoParticles.property("value", state.psoParticles);

  let psoOmega = d3.select("#psoOmega").on("change", function () {
    state.psoOmega = +this.value;
    state.serialize();
    userHasInteracted();
    parametersChanged = true;
  });
  psoOmega.property("value", state.psoOmega);

  let psoAlpha1 = d3.select("#psoAlpha1").on("change", function () {
    state.psoAlpha1 = +this.value;
    state.serialize();
    userHasInteracted();
    parametersChanged = true;
  });
  psoAlpha1.property("value", state.psoAlpha1);

  let psoAlpha2 = d3.select("#psoAlpha2").on("change", function () {
    state.psoAlpha2 = +this.value;
    state.serialize();
    userHasInteracted();
    parametersChanged = true;
  });
  ...
  window.addEventListener("resize", () => {
    let newWidth = document.querySelector("#main-part").getBoundingClientRect()
      .width;
